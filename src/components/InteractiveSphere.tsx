  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.z =
        position[2] + Math.cos(state.clock.elapsedTime * 0.4) * 0.2;

      // Transições de cor
      const targetColor = new Color(hovered ? "#bfc3d6" : "#d145c5");
      currentColor.current.lerp(targetColor, 5 * delta);

      // Transições de emissividade
      const targetEmissive = new Color(clicked ? "#888896" : "#2b1e1e");
